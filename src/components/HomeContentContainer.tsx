...
async function getCityPosition({
  ...
  if (city !== null && city !== undefined) {
    let response =
      (await fetch(
        `https://api.openweathermap.org/geo/1.0/direct?q=${city}&limit=1&appid=${setting?.apiKey}`
      )) || [];
...
async function getHourlyWeather({
  ...
  let response = await fetch(
    `https://api.openweathermap.org/data/2.5/forecast?lat=${latitude}&lon=${longitude}&units=${setting.metric}&appid=${setting.apiKey}`
  );
...
export default function HomeContentContainer({
  ...
  const [hourlyWeatherApiInfo, setHourlyWeatherApiInfo] = useState<
    Array<{
  ...
  async function dataFetching({ city }: { city: string | null }) {
    const city_location = await getCityPosition({
      city: city,
      setting: setting,
    });
  ...
  useEffect(() => {
    ...
    dataFetching({ city: city }).then((res) => {
      setWeatherApiInfo(res);
    });

    hourlyDataFetching({ city: city }).then((res) => {
  ...
  useEffect(() => {
    if (
      (weatherApiInfo?.city !== undefined || weatherApiInfo?.city !== null) &&
      setting?.user?.auto_refetch
    ) {
      // call every 5 minute
      let data_fetch_interval = setInterval(() => {
        console.info("start to re-fetch data");
        dataFetching({ city: weatherApiInfo?.city }).then((res) =>
          setWeatherApiInfo(res)
        );
  ...
  return (
    <div>
      <IonSearchbar
        ...
        onIonInput={(e) => setSelectedCity(e?.target?.value ?? null)}
        debounce={500}
        color="Light"
      />
      ...
      {weatherApiInfo?.city === undefined && (
        <WeatherCardError message="Nama tempat tidak ditemukan" />
      )}
      <IonGrid>
        {weatherApiInfo?.city !== undefined && (
          <IonRow>
